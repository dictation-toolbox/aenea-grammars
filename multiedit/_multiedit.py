...
#
# includes two sets of bindings: generic and vim. the latter is quite
# primitive and revolves around to spending all your time in insert
...
eclipse_command_table = aenea.raul.make_grammar_commands('multiedit', {
    ...
    'block [<n>]':          (Key('sa-up:%(n)d'),         None),
    'shrink [<n>]':         (Key('sa-down:%(n)d'),       None),
    'squirt [<n>]':         (Key('sa-right:%(n)d'),      None),
    'land [<n>]':           (Key('sa-left:%(n)d'),       None),
    'glide [<n>]':          (Key('a-right:%(n)d'),       None),
    'slip [<n>]':           (Key('a-left:%(n)d'),        None),
...
mapping = dict((key, val[0]) for (key, val) in command_table.iteritems())
vim_mapping = dict((key, val[1]) for (key, val) in command_table.iteritems())
